/**
 * 根据 User-Agent 获取响应 HTML 内容
 */
export function getResponseHtml(userAgent) {
...
function getInAppWarningPage() {
  return `
    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .card {
          width: 100%; max-width: 360px; background: var(--card-bg); border-radius: 26px;
          padding: 24px 22px 20px; border: 1px solid var(--card-border);
          backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
          box-shadow: 0 18px 45px rgba(15, 23, 42, 0.12), 0 0 0 1px rgba(255, 255, 255, 0.5);
          text-align: center; position: relative; animation: cardIn 0.45s cubic-bezier(0.2, 0.8, 0.2, 1);
        }
        .icon-wrap {
          width: 52px; height: 52px; border-radius: 18px; background: var(--primary-gradient);
        ...
        .pill {
          ...
          border: 1px solid rgba(148, 163, 184, 0.4); background: rgba(249, 250, 251, 0.9); margin-bottom: 8px;
        }
        ...
        .btn-main {
          border-radius: 999px; padding: 10px 14px; border: none; outline: none;
          font-size: 15px; font-weight: 500; color: #fff;
          background-image: var(--primary-gradient); box-shadow: 0 8px 18px rgba(37, 99, 235, 0.45);
        }
        .btn-secondary {
          border-radius: 999px; padding: 9px 12px; font-size: 13px; color: var(--text-secondary);
          background: #f9fafb; border: 1px solid rgba(209, 213, 219, 0.9);
        }
    ...
    <body>
      ...
      <div class="card">
        <div class="icon-wrap">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"></circle><polygon points="15.5 8.5 13.6 13.6 8.5 15.5 10.4 10.4 15.5 8.5"></polygon></svg>
        </div>
        <div class="pill">系统提示</div>
        <div class="title">当前环境不支持</div>
        <div class="desc">
          检测到您正在使用 <span class="highlight">微信或 QQ 内置浏览器</span> 访问。<br>
          为了保障支付安全和稳定，本页面暂不支持在当前环境中打开。
        </div>
        <ul class="steps">
          <li>1. 复制当前页面链接；</li>
          <li>2. 打开手机自带浏览器（如 Safari / 系统浏览器）；</li>
          <li>3. 在地址栏粘贴链接并访问。</li>
        </ul>
        <div class="btn-row">
          <button class="btn-main">请复制链接到浏览器打开</button>
          <div class="btn-secondary">如仍有问题，可在浏览器中联系在线客服</div>
        </div>
      </div>
    </body>
    </html>
...
function getRedirectPage() {
  return `
    ...
    <head>
      ...
      <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
      <title>疯果VPP</title>
      <script>console.log('Payment center page loaded');</script>
      <style>
        :root {
          --bg: #f3f4f6; --card-bg: rgba(255, 255, 255, 0.96); --card-border: rgba(15, 23, 42, 0.08);
          --text-primary: #111827; --text-secondary: #6b7280;
          --primary-gradient: linear-gradient(135deg, #0A84FF 0%, #5E5CE6 100%); --accent-soft: #e5edff;
        }
        * { box-sizing: border-box; }
        body { 
          font-family: "PingFang SC", "SF Pro Display", -apple-system, BlinkMacSystemFont, sans-serif;
        ...
        .title-pill {
          display: inline-flex; align-items: center; justify-content: center; padding: 3px 10px;
          border-radius: 999px; font-size: 11px; color: var(--text-secondary);
          border: 1px solid rgba(148, 163, 184, 0.4); background: rgba(249, 250, 251, 0.95); margin-bottom: 8px;
        }
        h1 {
          font-size: 28px; font-weight: 600; margin: 0;
          background: linear-gradient(to bottom, #111827 40%, #4b5563 100%);
        ...
        .nav-item {
          ...
          padding: 16px 18px; background: var(--card-bg); backdrop-filter: blur(18px); -webkit-backdrop-filter: blur(18px);
          border: 1px solid var(--card-border); border-radius: 20px; text-decoration: none; color: var(--text-primary);
          overflow: hidden; transition: all 0.2s cubic-bezier(0.25, 0.8, 0.25, 1); box-shadow: 0 10px 24px rgba(15, 23, 42, 0.08);
        }
        .nav-item::before {
          content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px;
          background: linear-gradient(90deg, transparent, rgba(148, 163, 184, 0.45), transparent); opacity: 0.6;
        }
        ...
        .tag-recommend {
          position: absolute; top: 10px; right: 12px; font-size: 10px; padding: 2px 8px; border-radius: 999px;
          background: #f97316; color: #fff; box-shadow: 0 4px 10px rgba(248, 113, 113, 0.4); display: none;
        }
        ...
        .footer-links {
          ...
          animation: fadeInUp 0.7s cubic-bezier(0.2, 0.8, 0.2, 1) 0.12s backwards;
        }
        .footer-btn {
          padding: 12px 24px; font-size: 16px; color: var(--text-secondary); background: #f9fafb;
          border-radius: 999px; text-decoration: none; border: 1px solid rgba(209, 213, 219, 0.9); transition: all 0.2s;
        }
    ...
    <body>
      ...
      <div class="container">
        <header>
          <div class="title-pill">疯果VPP · 安全导航</div>
          <h1>选择下单通道</h1>
          <div class="subtitle">已为你自动测试当前网络下各通道延迟，优先使用推荐线路</div>
        </header>

        <div class="nav-group">
          <a href="https://ap.apnfenguo.top/" class="nav-item" target="_blank" data-line-id="line1">
            <span class="tag-recommend" data-tag-for="line1">推荐</span>
            <div class="nav-content"><div class="nav-icon">⚡️</div><div><div class="nav-text-main">极速通道 ①</div><div class="nav-text-sub">优先推荐 · 较高成功率</div></div></div>
            <div class="nav-right"><span class="latency-label" data-latency-id="line1">测试中…</span><div class="nav-arrow">›</div></div>
          </a>
          <a href="https://xx.fenguobom.top/" class="nav-item" target="_blank" data-line-id="line2">
            <span class="tag-recommend" data-tag-for="line2">推荐</span>
            <div class="nav-content"><div class="nav-icon">💎</div><div><div class="nav-text-main">备用通道 ②</div><div class="nav-text-sub">线路优化 · 稳定性更佳</div></div></div>
            <div class="nav-right"><span class="latency-label" data-latency-id="line2">测试中…</span><div class="nav-arrow">›</div></div>
          </a>
          <a href="https://www.payvpp.top/" class="nav-item" target="_blank" data-line-id="line3">
            <span class="tag-recommend" data-tag-for="line3">推荐</span>
            <div class="nav-content"><div class="nav-icon">🛡️</div><div><div class="nav-text-main">备用通道 ③</div><div class="nav-text-sub">前两条无法使用时再尝试</div></div></div>
            <div class="nav-right"><span class="latency-label" data-latency-id="line3">测试中…</span><div class="nav-arrow">›</div></div>
          </a>
        </div>

        <div class="footer-links">
          <a href="https://tawk.to/chat/66addd061601a2195ba04b79/1i4bieonn" class="footer-btn" target="_blank">在线客服</a>
          <a href="/course.html" class="footer-btn" target="_blank">使用教程</a>
        </div>
      </div>
      ...
      <script>
        (function () {
          ...
          function testLatency(line) {
            ...
            return fetch(line.url, { method: 'GET', mode: 'no-cors', cache: 'no-store' }).catch(() => null).then(() => {
              const end = performance.now();
              const latency = Math.round(end - start);
              return (!isFinite(latency) || latency <= 0) ? null : latency;
            });
          }
          async function runTests() {
            ...
            await Promise.all(lines.map(async (line) => {
              ...
              if (!labelEl) return;
              if (latency === null) {
                labelEl.textContent = '不可用'; labelEl.style.background = '#fee2e2'; labelEl.style.color = '#b91c1c';
              } else {
            ...
            if (bestCard && tag) { bestCard.classList.add('recommended'); tag.style.display = 'inline-block'; }
          }
